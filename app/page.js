...
const EMPTY_FORM = {
  nama_lengkap: '', no_ktp: '', no_kk: '', pembayaran: 'cash',
  nama_motor: '', serial_produk: '', status_leasing: 'processed',
  phone: '', tanggal: new Date().toISOString().split('T')[0],
  nama_sales: '', catatan: ''
}
...
export default function Home() {
  ...
  const [quickStatus, setQuickStatus] = useState(null) // id of card with open quick-status

  useEffect(() => { fetchCustomers() }, [])

  async function fetchCustomers() {
    const { data } = await supabase.from('customers').select('*').order('tanggal', { ascending: false })
    setCustomers(data || [])
  }
  ...
  function handleEdit(c) {
    setForm({
      nama_lengkap: c.nama_lengkap, no_ktp: c.no_ktp, no_kk: c.no_kk,
      pembayaran: c.pembayaran, nama_motor: c.nama_motor, serial_produk: c.serial_produk,
      status_leasing: c.status_leasing, phone: c.phone || '',
      tanggal: c.tanggal || new Date().toISOString().split('T')[0],
      nama_sales: c.nama_sales || '', catatan: c.catatan || ''
    })
    setEditingId(c.id)
    setShowForm(true)
  ...
  async function handleQuickStatus(id, status) {
    await supabase.from('customers').update({ status_leasing: status }).eq('id', id)
    setQuickStatus(null)
    fetchCustomers()
  }
  ...
  const filtered = customers.filter(c => {
    ...
    const matchSearch = !q || c.nama_lengkap?.toLowerCase().includes(q) || c.phone?.toLowerCase().includes(q) || c.nama_sales?.toLowerCase().includes(q)
    const matchStatus = filterStatus === 'all' || c.status_leasing === filterStatus
    return matchSearch && matchStatus
  })

  const f = (key, lbl, col = 'span 1') => ({ key, lbl, col })
  const textFields = [
    f('nama_lengkap', 'Nama Lengkap', 'span 2'),
    f('no_ktp', 'No. KTP'),
    f('no_kk', 'No. KK'),
    f('phone', 'No. Telepon'),
    f('nama_sales', 'Nama Sales'),
    f('nama_motor', 'Nama Motor'),
    f('serial_produk', 'Serial Produk'),
  ]
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input, select, textarea {
          outline: none; background: #fff; border: 1px solid #e4e4e7;
          color: #111; border-radius: 8px; padding: 9px 12px;
          width: 100%; font-size: 13px; font-family: 'Geist', sans-serif;
          transition: border-color 0.15s, box-shadow 0.15s;
        }
        input::placeholder, textarea::placeholder { color: #a1a1aa; }
        input:focus, select:focus, textarea:focus { border-color: #111; box-shadow: 0 0 0 3px rgba(0,0,0,0.06); }
      ...
      <main style={{ maxWidth: 700, margin: '0 auto', padding: '36px 16px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
          ...
          <button
            ...
            style={{ background: showForm ? '#f4f4f5' : '#111', color: showForm ? '#71717a' : '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          <input
            placeholder="Search name, sales, phone..."
            value={search}
          ...
          <div style={{ background: '#fff', border: '1px solid #e4e4e7', borderRadius: 12, padding: 20, marginBottom: 16, boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              ...
              <div style={{ gridColumn: 'span 2' }}>
                <label>Catatan</label>
                <textarea placeholder="Catatan tambahan..." value={form.catatan} onChange={e => setForm({ ...form, catatan: e.target.value })} />
              </div>
            </div>
            <button onClick={handleSubmit}
              style={{ marginTop: 14, width: '100%', background: '#111', color: '#fff', border: 'none', borderRadius: 8, padding: '10px', fontSize: 13, fontWeight: 600 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.length === 0 && (
            <div style={{ textAlign: 'center', color: '#d4d4d8', padding: '48px 0', fontSize: 13 }}>No customers found</div>
          )}
          {filtered.map(c => {
            const badge = STATUS_BADGE[c.status_leasing]
            const isQuick = quickStatus === c.id
            return (
              <div key={c.id} style={{ background: '#fff', border: '1px solid #e4e4e7', borderRadius: 12, padding: '14px 16px', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: '#a1a1aa', marginTop: 1 }}>{c.nama_sales && `Sales: ${c.nama_sales}`}{c.tanggal && ` · ${c.tanggal}`}</p>
                  </div>
                ...
                {/* Info grid */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '5px 16px', marginBottom: 10 }}>
                  {[
                    ['KTP', c.no_ktp],
                    ['KK', c.no_kk],
                    ['Motor', c.nama_motor],
                    ...
                    <div key={lbl} style={{ display: 'flex', gap: 6, alignItems: 'baseline' }}>
                      <span style={{ fontSize: 10, color: '#a1a1aa', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.06em', minWidth: 38 }}>{lbl}</span>
                ...
                {/* Catatan */}
                {c.catatan && (
                  <div style={{ background: '#fafafa', border: '1px solid #f0f0f0', borderRadius: 7, padding: '7px 10px', marginBottom: 10 }}>
                    <p style={{ fontSize: 11, color: '#a1a1aa', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 2 }}>Catatan</p>
                    <p style={{ fontSize: 12, color: '#52525b' }}>{c.catatan}</p>
                  </div>
                )}
                ...
                {isQuick && (
                  <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                    {['processed', 'acc', 'reject'].map(s => (
                      <button key={s} onClick={() => handleQuickStatus(c.id, s)}
                        style={{ flex: 1, padding: '6px', fontSize: 11, fontWeight: 600, borderRadius: 7, border: 'none', textTransform: 'uppercase', letterSpacing: '0.06em', background: STATUS_BADGE[s].bg, color: STATUS_BADGE[s].color }}>
                ...
                <div style={{ display: 'flex', gap: 6, borderTop: '1px solid #f4f4f5', paddingTop: 10 }}>
                  <button onClick={() => setQuickStatus(isQuick ? null : c.id)}
